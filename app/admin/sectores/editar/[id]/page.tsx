...
export default function EditarSectorPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      setForm({
        ...
        featured: sector.featured ?? false,
      });
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#0a1628] text-white">
        <div className="rounded-[2rem] border border-[#009B8D]/15 bg-[#0f2744] p-8 text-center shadow-2xl">
          ...
          <p className="font-semibold text-slate-400">Cargando sector...</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <AdminEditorShell
      title="Editar sector"
      description="Actualiza el sector público, su ícono, descripción, orden, visibilidad y marca de destacado."
