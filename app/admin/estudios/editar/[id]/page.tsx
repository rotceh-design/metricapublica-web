...
export default function EditarEstudioPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      const estudioWithImages = estudio as EstudioEditorForm;
      ...
      setForm({
        ...
        year: estudio.year || new Date().getFullYear().toString(),
        status: estudio.status || "draft",
        imageUrl: estudio.imageUrl || "",
        pdfUrl: estudio.pdfUrl || "",
        pdfName: estudio.pdfName || "",
        imageFit: estudio.imageFit || "contain",
        imagePosition: estudio.imagePosition || "center",
        imageAspect: estudio.imageAspect || "wide",
        methodology: estudio.methodology || "",
        sample: estudio.sample || "",
        fieldworkDate: estudio.fieldworkDate || "",
        images: estudioWithImages.images || [],
      });

      const draftsFromContent = buildImageDraftsFromContent(
        estudioWithImages.images,
        {
          imageUrl: estudio.imageUrl,
          imageAspect: estudio.imageAspect,
          imageFit: estudio.imageFit,
          imagePosition: estudio.imagePosition,
        }
      );

      setImageDrafts(normalizeDrafts(draftsFromContent));
      setLoading(false);
    });
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await updateEstudio(id, payload);
  ...
  return (
    <AdminEditorShell
      ...
      description="Edita la publicación descargable con portada, imagen de apoyo, PDF, ficha técnica y contenido descriptivo."
    ...
    >
      <form id="edit-estudio-form" onSubmit={handleSubmit}>
        ...
        <div className="grid w-full gap-5 2xl:grid-cols-[minmax(760px,1.45fr)_minmax(420px,0.65fr)]">
          <section className="space-y-5">
            ...
            {saving && imageUploadProgress.total > 0 && (
              <ProgressPanel
